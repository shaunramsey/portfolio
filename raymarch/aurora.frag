...
//aurora.frag
vec3 aurora(vec3 ori, vec3 dir, float t) {
  t = 500.0 / -dir.z; //howl ong does it take z to go 500.0
  float dy = dir.y;
  vec3 pos = ori + dir*t;
  //pos = pos * 0.01;
  float maxheight = eye.y + T_END;
  float minheight = eye.y - 0.0*T_END;
  float height = 1.0-(pos.y - minheight)/(maxheight-minheight);
  float aspect = 640.0/360.0;
  float left = eye.x - aspect*T_END;
  float right = eye.x + aspect*T_END;
  float x = (pos.x - left)/(right-left);
  float time = iTime*10.0; // 3.14159*0.5;
  ...
  float noise2 = exp(- (noised1(pos.xxx*0.01).x) + upper*upper) * upper;
